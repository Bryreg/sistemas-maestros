...
metadata:
  author: sistemas-maestros
  version: "1.0.0"
  origin: "Adaptado desde retail-espacios/.claude/skills/ui-ux-pro-max"
---
...
Recommended to use this skill when:

- UI looks "unprofessional" but the reason isn't clear
- Receiving usability or experience-related feedback
- Preparing UI quality improvements before launch
- Need to align cross-platform design (Web / iOS / Android)
- Building a design system or reusable component library

### Skip

No need to use this skill when:
...
**Rule of thumb**: if a task changes how something **looks, feels, moves, or is interacted with**, use this skill.

## Rule Categories by Priority

*For human/AI reference: use 1→10 to decide what to focus on first; use `--domain <Domain>` to query details. Scripts do not read this table.*
...
- When the recommendation table has no fitting icon:
  - **Prefer picking any more semantically fitting icon from the full Phosphor set first**;
  - If Phosphor also has no ideal option, use **Heroicons (`@heroicons/react`)** as a fallback, keeping the style consistent (linear/filled, stroke weight, corner radius).
